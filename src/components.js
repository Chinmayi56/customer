...
function priceLabel(p) {
  return p.price ? `₹${Number(p.price).toLocaleString('en-IN')}` : 'Contact for Price';
}
  ...
  <article class="group bg-white rounded-2xl border border-fc-line shadow-card hover:shadow-soft transition-shadow duration-300 overflow-hidden flex flex-col">
    ...
    <div class="p-4 flex flex-col gap-2 flex-1">
      ...
      <div class="mt-auto pt-3 flex items-center justify-between">
        <span class="font-display font-semibold text-fc-charcoal">${priceLabel(p)}</span>
      </div>
      <div class="flex gap-2 pt-1">
        ...
        <button data-getcode="${p.id}" class="flex-1 text-sm font-medium bg-fc-green text-white rounded-xl py-2 hover:bg-fc-greendark transition-colors">Get a Code</button>
      </div>
    </div>
  </article>`;
}
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="h-16 flex items-center justify-between gap-4">
        <a href="#/" class="flex items-center gap-2 shrink-0">
          <img src="${COMPANY.logo}" alt="Farm Craft" class="h-9 w-9 rounded-lg object-cover" />
